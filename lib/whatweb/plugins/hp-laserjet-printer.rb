...
##
WhatWeb::Plugin.define "HP-LaserJet-Printer" do
  ...
  def passive(target)
    ...
    m << { version: target.headers['server'].scan(/HP-ChaiServer\/([\d\.]+)/) } if target.headers['server'] =~ /HP-ChaiServer\/([\d\.]+)/

    # Server: HP-ChaiSOE
    m << { version: target.headers['server'].scan(/HP-ChaiSOE\/([\d\.]+)/) } if target.headers['server'] =~ /HP-ChaiSOE\/([\d\.]+)/

    # Location: hp/device/this.LCDispatcher
    m << { name: "HTTP Location Header" } if target.headers['location'] =~ /hp\/device\/this.LCDispatcher/
