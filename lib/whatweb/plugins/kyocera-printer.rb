...
##
WhatWeb::Plugin.define "Kyocera-Printer" do
  ...
  def passive(target)
    ...
    if /^KM-MFP-http\/V([\d\.]+)$/.match?(target.headers["server"])

      # Model Detection
      m << { model: target.body.scan(/^var ModelName="([^"]+)";/) } if target.body =~ /^var ModelName="([^"]+)";/
