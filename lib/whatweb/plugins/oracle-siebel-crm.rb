...
##
WhatWeb::Plugin.define "Oracle-Siebel-CRM" do
  ...
  def passive(target)
    ...
    # Cookies
    m << { name: "BIGipServerSiebel_EService_Prod Cookie" } if target.headers["set-cookie"] =~ /BIGipServerSiebel_EService_Prod=[\d\.]+;/
    m << { name: "BIGipServerSIEBEL Cookie" } if target.headers["set-cookie"] =~ /BIGipServerSIEBEL-80=[\d\.]+;/
    m << { name: "_sn Cookie" } if target.headers["set-cookie"] =~ /_sn=[a-zA-Z0-9]{43}_; Version=1; Path=\//

    # Landing page # Default form HTML
    m << { string: target.body.scan(/<input type[\s]*=[\s]*['"]?hidden["']? name[\s]*=[\s]*['"]?SWEView['"]? value[\s]*=[^\(]+\(([^\)]*)\)/).flatten } if target.body =~ /<input type[\s]*=[\s]*['"]?hidden["']? name[\s]*=[\s]*['"]?SWEView['"]? value[\s]*=[^\(]+\(([^\)]*)\)/
