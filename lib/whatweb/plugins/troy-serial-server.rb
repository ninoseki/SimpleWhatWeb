...
##
WhatWeb::Plugin.define "TROY-Serial-Server" do
  ...
  def passive(target)
    ...
    m << { model: "TROY500", name: "HTTP Server & Date Header" } if target.headers["server"] =~ /^XCD Web Admin$/ && target.headers["file-created"] =~ /^Wed, 14 Jul 1999 04:00:00 GMT$/
