...
##
WhatWeb::Plugin.define "HP-System-Management-Homepage" do
  ...
  def passive(target)
    ...
    if /<TITLE>HP System Management Homepage Login<\/TITLE>/.match?(target.body)
      if target.body =~ /systemname = "([^\"^\s]+)";[\s]+smhversion = "HP System Management Homepage v([^\"^\s]+)";[\s]+smhcopyright = "&copy;2003-20[\d]{2} Hewlett-Packard Development Company, L.P.";/
        m << { string: $1.to_s }
        m << { version: $2.to_s }
      end
    end
