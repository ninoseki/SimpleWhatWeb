...
##
WhatWeb::Plugin.define "ZyXEL-Router" do
  ...
  def passive(target)
    ...
    if /^ZyXEL-RomPager/.match?(target.headers["server"])
      ...
      m << { model: target.headers["www-authenticate"].scan(/^Basic realm="(Prestige [^"]+)( Web)?"/)[0][0] } if target.headers["www-authenticate"] =~ /^Basic realm="(Prestige [^"]+)( Web)?"/
    ...
    # HTTP Server Header # RomPager
    if /^RomPager/.match?(target.headers["server"])
    ...
    if target.headers["www-authenticate"] =~ /Basic realm="P-330W EE \(username: admin\)"/ && target.headers["server"] =~ /GoAhead-Webs/ && @status.to_s =~ /^401$/
      m << { model: "P-330W EE" }
    end
