...
##
WhatWeb::Plugin.define "NeXpose-Security-Console" do
  ...
  def aggressive(target)
    ...
    url = URI.join(target.uri.to_s, "/style/").to_s
    new_target = WhatWeb::Target.new(url)
    m << { name: "Default Error Page Title" } if new_target.body =~ /<title>NeXpose Security Console :: Error<\/title>/
