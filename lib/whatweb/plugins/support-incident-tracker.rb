...
##
WhatWeb::Plugin.define "Support-Incident-Tracker" do
  ...
  def passive(target)
    ...
    if /<!-- SiT \(Support Incident Tracker\) - Support call tracking system[\s]+Copyright \(C\) (20[\d]{2}-)?(20[\d]{2}) (The Support Incident Tracker Project|Salford Software Ltd\. and Contributors)/.match?(target.body)

      # Year Detection # HTML Comment # Copyright Notice
      m << { string: target.body.scan(/<!-- SiT \(Support Incident Tracker\) - Support call tracking system[\s]+Copyright \(C\) (20[\d]{2}-)?(20[\d]{2}) (The Support Incident Tracker Project|Salford Software Ltd\. and Contributors)/)[0][1] }

      # Version Detection # Meta Generator
      m << { version: target.body.scan(/<meta name="GENERATOR" content="[^"]+ v([^"]+)" \/>/) } if target.body =~ /<meta name="GENERATOR" content="[^"]+ v([^"]+)" \/>/

    end

    # SiTsessionID Cookie
    m << { name: "SiTsessionID Cookie" } if target.headers["set-cookie"] =~ /SiTsessionID=[^;^\s]+/
