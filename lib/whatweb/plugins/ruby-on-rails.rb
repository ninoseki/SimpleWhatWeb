...
##
WhatWeb::Plugin.define "Ruby-on-Rails" do
  ...
  def passive(target)
    m = []
    m << { name: '5 JS files', certainty: 75 } if target.body =~ /javascripts\/prototype\.js\?[0-9]+/ && target.body =~ /javascripts\/effects\.js\?[0-9]+/ && target.body =~ /javascripts\/dragdrop\.js\?[0-9]+/ && target.body =~ /javascripts\/controls\.js\?[0-9]+/ && target.body =~ /javascripts\/application\.js\?[0-9]+/

    m << { name: "X-Powered-By: Phusion Passenger" } if target.headers["x-powered-by"] =~ /Phusion Passenger/ # Phusion is only for rails
    m
  end
end
