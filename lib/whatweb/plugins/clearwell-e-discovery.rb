...
##
WhatWeb::Plugin.define "Clearwell-E-Discovery" do
  ...
  def passive(target)
    ...
    # HTTP Server Header
    if /^Clearwell$/.match?(target.headers["server"])

      m << { name: "HTTP Server Header" }

      # Version Detection
      m << { version: target.body.scan(/<p class="build">v([^<]+)<\/p>/) } if target.body =~ /<p class="build">v([^<]+)<\/p>/
