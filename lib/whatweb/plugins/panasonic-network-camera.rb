...
# Fixed regex
##
WhatWeb::Plugin.define "Panasonic-Network-Camera" do
  ...
  def passive(target)
    ...
    # Version Detection
    if /      <FONT FACE="Arial" STYLE="font-size: 16px" COLOR="#ffffff">/.match?(target.body)
      if /        <B>([\d\-A-Z]+)<\/B>/.match?(target.body)
        version = target.body.scan(/        <B>([\d\-A-Z]+)<\/B>/)[0][0]
        m << { version: version }
      end
    end
