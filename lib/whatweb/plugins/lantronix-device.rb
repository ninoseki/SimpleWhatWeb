...
##
WhatWeb::Plugin.define "Lantronix-Device" do
  ...
  def passive(target)
    ...
    # HTTP Server Header
    if /^Gordian Embedded([\d\.]+)$/.match?(target.headers["server"])

      # Model Detection
      m << { model: target.body.scan(/<font face="Arial,Helvetica" color="#660066"><b>([^<]+)<\/b><\/font><br><br>/) } if target.body =~ /<font face="Arial,Helvetica" color="#660066"><b>([^<]+)<\/b><\/font><br><br>/
