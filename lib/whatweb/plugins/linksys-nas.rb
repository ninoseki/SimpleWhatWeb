...
# Fixed regex
##
WhatWeb::Plugin.define "Linksys-NAS" do
  ...
  def passive(target)
    ...
    if /	 <td nowrap class="hst">&nbsp; &nbsp;Version:<\/td>/.match?(target.body)
      if /	          <td nowrap class="hst">V([\d\.a-zA-Z]+)<\/td>/.match?(target.body)
        version = target.body.scan(/	          <td nowrap class="hst">V([\d\.a-zA-Z]+)<\/td>/)[0][0]
        m << { version: version }
      end
    end
