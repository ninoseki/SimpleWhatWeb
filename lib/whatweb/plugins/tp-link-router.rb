...
##
WhatWeb::Plugin.define "TP-Link-Router" do
  ...
  def passive(target)
    ...
    # HTTP Server Header
    m << { name: "HTTP Server" } if target.headers['server'] =~ /TP\-LINK Router/

    # WWW-Authenticate # Model Detection
    m << { model: target.headers['www-authenticate'].scan(/Basic realm="TP\-LINK ([^\"]*)"/) } if target.headers['www-authenticate'] =~ /Basic realm="TP\-LINK ([^\"]*)"/
