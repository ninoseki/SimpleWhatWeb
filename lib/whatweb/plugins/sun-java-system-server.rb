...
##
WhatWeb::Plugin.define "Sun-Java-System-Server" do
  ...
  def passive(target)
    ...
    if /^[\s]*Sun[\-\ ]{1}Java[\-\ ]{1}System[\/\ ]{1}Application[\-\ ]{1}Server/.match?(target.headers["server"].to_s)
      m << if /^[\s]*Sun[\-\ ]{1}Java[\-\ ]{1}System[\/\ ]{1}Application[\-\ ]{1}Server ([\d\._]+)/.match?(target.headers["server"].to_s)
             { version: target.headers["server"].scan(/^[\s]*Sun[\-\ ]{1}Java[\-\ ]{1}System[\/\ ]{1}Application[\-\ ]{1}Server ([\d\._]+)/).flatten, module: "Application Server" }
           else
             { name: "HTTP server header", module: "Application Server" }
           end
