...
##
WhatWeb::Plugin.define "Nintendo-GameCube" do
  ...
  def passive(target)
    ...
    # Server: GameCube
    m << { certainty: 75, name: "Server Header" } if target.headers["server"] =~ /^Game[\s]?Cube$/

    # X-Organization: Nintendo
    m << { certainty: 75, name: "X-Organization Header" } if target.headers["x-organization"] =~ /^Nintendo$/
