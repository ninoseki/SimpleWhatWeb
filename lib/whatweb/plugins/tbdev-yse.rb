...
##
WhatWeb::Plugin.define "TBDev-YSE" do
  ...
  def passive(target)
    ...
    # X-Powered-By header
    m << { name: "X-Powered-By header" } if target.headers["x-powered-by"] =~ /^TBDev Yuna Scatari Edition/
    m << { name: "X-Powered-By header" } if target.headers["x-powered-by"] =~ /^TBDev YSE/

    # X-Chocolate-to header
    m << { name: "X-Chocolate-to header" } if target.headers["x-chocolate-to"] =~ /^ICQ 7282521$/
