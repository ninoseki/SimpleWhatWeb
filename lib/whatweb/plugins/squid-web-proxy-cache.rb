...
# Updated regex
##
WhatWeb::Plugin.define "Squid-Web-Proxy-Cache" do
  ...
  def passive(target)
    ...
    m << { name: "x-squid-error", certainty: 25 } unless target.headers["x-squid-error"].nil?

    # Server
    m << { version: target.headers["server"].to_s.scan(/[\s]*Squid\/([^\r^\n]+)/i).flatten } if target.headers["server"] =~ /[\s]*Squid\/([^\r^\n]+)/i
