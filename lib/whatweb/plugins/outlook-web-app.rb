...
##
WhatWeb::Plugin.define "Outlook-Web-App" do
  ...
  def passive(target)
    ...
    m << { version: target.headers["x-owa-version"].to_s } unless target.headers["x-owa-version"].nil?

    # Active Directory Domain Detection
    if /<!--Copyright \(c\) 2000-20[\d]{2} Microsoft Corporation\.  All rights reserved\.-->/.match?(target.body)
      if target.body =~ /logonForm\.username\.value[\s]*=[\s]*"([^"^\\]+)\\\\"/i
        m << { string: "AD Domain: #{$1}" }
      elsif target.body =~ /document\.getElementById\("username"\)\.value = '([^']+)'/i
        m << { string: "AD Domain: #{$1}" }
      end
    end
