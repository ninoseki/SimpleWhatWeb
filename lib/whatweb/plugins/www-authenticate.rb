...
# Added examples
##
WhatWeb::Plugin.define "WWW-Authenticate" do
  ...
  def passive(target)
    ...
    unless target.headers['www-authenticate'].nil?

      # Detect Realm
      if /realm=/.match?(target.headers['www-authenticate'].to_s)

        # Extract Authentication Method
        m << { module: target.headers['www-authenticate'].scan(/([a-z]{3,20})[^\r^\n]{1,256}realm="/i).flatten } if target.headers['www-authenticate'].to_s =~ /[a-z]{3,20}[^\r^\n]{1,256}realm="/i

        # Extract Authentication Realm
        m << { string: target.headers['www-authenticate'].scan(/realm="([^\"]{1,256})"/i).flatten } if target.headers['www-authenticate'].to_s =~ /realm="[^\"]{1,256}"/i

      else

        # No Realm # Extract Authentication Method
        m << { module: target.headers['www-authenticate'].to_s }
