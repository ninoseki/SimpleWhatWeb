...
##
WhatWeb::Plugin.define "Horde-Application-Framework" do
  ...
  def passive(target)
    ...
    m << { name: "webmail_version Cookie" } if target.headers["set-cookie"] =~ /webmail_version=(magnum|hunter); /

    # webmail4prod Cookie
    m << { name: "webmail4prod Cookie" } if target.headers["set-cookie"] =~ /webmail4prod=[a-f\d]{32}; /
