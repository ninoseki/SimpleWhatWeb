...
##
WhatWeb::Plugin.define "Sony-Projector" do
  ...
  def passive(target)
    ...
    # HTTP Server
    if /^Network Projector$/.match?(target.headers["server"])
      m << { certainty: 75, name: "Server Header" }
    end

    # WWW-Authenticate Header
    if /Basic realm="Projector (User|Administration)"/.match?(target.headers["www-authenticate"])
