...
##
WhatWeb::Plugin.define "Novell-NetWare" do
  ...
  def passive(target)
    ...
    # HTTP Server Header
    if /^NetWare/.match?(target.headers["server"])

      if /^NetWare HTTP Stack$/.match?(target.headers["server"])
        m << { name: "HTTP Server Header" }
      end

      if target.headers["server"] =~ /^NetWare-Enterprise-Web-Server\/([^\s]+)$/
        m << { version: $1.to_s }
      end
