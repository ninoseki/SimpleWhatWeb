...
##
WhatWeb::Plugin.define "Card-Reader-Adapter" do
  ...
  def passive(target)
    ...
    if (@status == 401) && target.headers["www-authenticate"] =~ /^Basic realm="System Login"$/ && (target.headers["server"] == "httpd") && target.body =~ /^<HTML> <Title> 401 unAuthorized <\/title>                   <body> <H1> 401 unauthorized request <\/H1><\/body>                   <\/HTML>$/
