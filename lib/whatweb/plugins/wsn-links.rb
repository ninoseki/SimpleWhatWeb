...
##
WhatWeb::Plugin.define "WSN-Links" do
  ...
  def passive(target)
    ...
    # Version Detection
    if /<title>WSN Links<\/title>/.match?(target.body)

      # Error Page
      if target.body =~ /<p>WSN Links is unable to render this page. The reason is described below:<\/p>/ && target.body =~ /<p>Version info: WSN Links ([\d\.]+)<\/p>/
        m << { version: target.body.scan(/<p>Version info: WSN Links ([\d\.]+)<\/p>/) }
      end

      # Error Page # no id parameter
      if target.body =~ /<div class="main">Non-numeric id value given in creating object.<\/div>/ && target.body =~ /<p>WSN Links ([\d\.]+)<\/p>/
        m << { version: target.body.scan(/<p>WSN Links ([\d\.]+)<\/p>/) }
      end
