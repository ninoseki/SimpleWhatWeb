...
##
WhatWeb::Plugin.define "Harris-NetVX" do
  ...
  def passive(target)
    ...
    if target.headers["server"] =~ /^GoAhead-Webs$/ && target.body =~ /<title>([^\s]+) Control<\/title>/
      m << { model: target.body.scan(/<title>([^\s]+) Control<\/title>/) }
    end
