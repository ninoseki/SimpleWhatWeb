...
##
WhatWeb::Plugin.define "Wordpress-Contact-Form" do
  ...
  def passive(target)
    ...
    if /<input type="hidden" name="_wpcf([\d]+)_version" value="[\d\.]+" \/>/.match?(target.body)
      version = target.body.scan(/<input type="hidden" name="_wpcf([\d]+)_version" value="([\d\.]+)" \/>/)[0][0] + "." + target.body.scan(/<input type="hidden" name="_wpcf[\d]+_version" value="([\d\.]+)" \/>/)[0][0]
      m << { version: version }
    end
