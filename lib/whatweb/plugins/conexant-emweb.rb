...
##
WhatWeb::Plugin.define "Conexant-EmWeb" do
  ...
  def passive(target)
    ...
    if target.headers["server"] =~ /Conexant-EmWeb\/([^\r^\n]+)/ || target.headers["server"] =~ /Virata-EmWeb\/([^\r^\n]+)/
      ...
      m << { model: "e-con " + target.body.scan(/<img src="\/webconfig\/images\/logo.gif" alt="LOGO" title="LOGO" hspace="0" vspace="0" width="220" height="45"><\/td><td class="rbtop" width="100%"><h1 class="pname">e-con ([^\s]+) Ethernet Router<\/h1>/).flatten.first } if target.body =~ /<img src="\/webconfig\/images\/logo.gif" alt="LOGO" title="LOGO" hspace="0" vspace="0" width="220" height="45"><\/td><td class="rbtop" width="100%"><h1 class="pname">e-con ([^\s]+) Ethernet Router<\/h1>/
  ...
  def aggressive(target)
    ...
    if target.headers["server"] =~ /Conexant-EmWeb\/([^\r^\n]+)/ || target.headers["server"] =~ /Virata-EmWeb\/([^\r^\n]+)/
      ...
      url = URI.join(target.uri.to_s, "/status/Status_1.htm").to_s
      new_target = WhatWeb::Target.new(url)

      # Allied Telesyn # Status page # Model Detection
      m << { url: "/status/Status_1.htm", model: "Allied Telesyn-" + new_target.body.scan(/<TD WIDTH="30%"><STRONG>Model Type<\/STRONG><\/TD>[\r\n\s]*<TD>([^<]+)<\/TD>/).flatten } if new_target.body =~ /<TD WIDTH="30%"><STRONG>Model Type<\/STRONG><\/TD>[\r\n\s]*<TD>([^<]+)<\/TD>/

      # Allied Telesyn # Status page # MAC Address
      m << { url: "/status/Status_1.htm", string: new_target.body.scan(/<TD><STRONG>MAC Address<\/STRONG><\/TD>[\r\n\s]*<TD>([^<]+)<\/TD>/).flatten } if new_target.body =~ /<TD><STRONG>MAC Address<\/STRONG><\/TD>[\r\n\s]*<TD>([^<]+)<\/TD>/

      # Allied Telesyn # iMG634A-R2 # Top frame
      url = URI.join(target.uri.to_s, "/webconfig/atl/top_frame.html").to_s
      new_target = WhatWeb::Target.new(url)
      m << { url: "/webconfig/atl/top_frame.html", model: new_target.body.scan(/^[\s]+<em>([^<]+)<\/em>/).flatten } if new_target.body =~ /^[\s]+<em>([^<]+)<\/em>/

      # Allied Telesyn # iMG646BD / iMG606BD # Top frame
      url = URI.join(target.uri.to_s, "/ati_header_frame.html").to_s
      new_target = WhatWeb::Target.new(url)
      m << { url: "/ati_header_frame.html", model: new_target.body.scan(/^[\s]+<em>([^<]+)<\/em>/).flatten } if new_target.body =~ /^[\s]+<em>([^<]+)<\/em>/
