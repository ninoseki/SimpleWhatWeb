...
##
WhatWeb::Plugin.define "SpinetiX-Hyper-Media-Player" do
  ...
  def passive(target)
    ...
    # HTTP Headers
    if target.headers["x-spinetix-firmware"] =~ /^([^\r^\n]*)/ && target.headers["x-raperca-version"] =~ /^([^\r^\n]*)/

      # X-spinetix-firmware
      m << { firmware: target.headers["x-spinetix-firmware"].scan(/^([^\r^\n]*)/).flatten, model: "HMP100" }

      # X-raperca-version
      m << { module: "raperca/" + target.headers["x-raperca-version"].scan(/^([^\r^\n]*)/).flatten, model: "HMP100" }

    end

    # Admin Page # Extract MAC
    if target.body =~ /<h1>SpinetiX HMP100 Web Access<\/h1><\/div>/ && target.body =~ /SpinetiX SA. All rights reserved.<br\/>/ && target.body =~ /<tr><td>MAC : <\/td><td>([^<]+)<\/td>/
      m << { account: target.body.scan(/<tr><td>MAC : <\/td><td>([^<]+)<\/td>/).flatten, model: "HMP100", string: "Admin" }
    end
