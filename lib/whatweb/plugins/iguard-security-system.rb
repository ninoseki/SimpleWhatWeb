...
##
WhatWeb::Plugin.define "iGuard-Security-System" do
  ...
  def passive(target)
    ...
    # HTTP Server Header
    if target.headers['server'] =~ /^iGuard Embedded Web Server\/([^\s]{1,15}) \(([^\s^\)]+)\) (SN:[^\s]+)$/
      m << { firmware: $1.to_s }
      m << { model: $2.to_s }
      m << { string: $3.to_s }
    end
    ...
    m << { name: "WWW-Authenticate Header" } if target.headers["www-authenticate"] =~ /Basic realm="iGuard System"/
