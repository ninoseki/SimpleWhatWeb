...
##
WhatWeb::Plugin.define "Microsoft-Sharepoint" do
  ...
  def passive(target)
    ...
    m << { version: target.headers["microsoftsharepointteamservices"].to_s } unless target.headers["microsoftsharepointteamservices"].nil?

    # x-sharepointhealthscore HTTP Header
    m << { name: "x-sharepointhealthscore HTTP header" } unless target.headers["x-sharepointhealthscore"].nil?
    ...
    m << { certainty: 25, name: "exires HTTP header" } unless target.headers["exires"].nil?
