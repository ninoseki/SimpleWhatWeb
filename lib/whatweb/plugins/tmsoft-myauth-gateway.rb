...
##
WhatWeb::Plugin.define "TMSoft-MyAuth-Gateway" do
  ...
  def passive(target)
    ...
    # X-Software-*
    if target.headers["x-software-info"] =~ /MyAuth Gateway/ && target.headers["x-software-owner"] =~ /Patrick Brandao, contato@tmsoft.com.br/

      m << { name: "X-Software HTTP Headers" }
      m << { version: target.headers["x-software-info"].scan(/MyAuth Gateway ([\d\.]+), TMSoft Solucoes \(www.tmsoft.com.br\)/) } if target.headers["x-software-info"] =~ /MyAuth Gateway ([\d\.]+), TMSoft Solucoes \(www.tmsoft.com.br\)/
      m << { version: target.headers["x-software-version"].scan(/([^\r^\n]*)/) } if target.headers["x-software-version"] =~ /([^\r^\n]*)/
