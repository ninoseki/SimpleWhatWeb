...
##
WhatWeb::Plugin.define "c99-Shell" do
  ...
  def passive(target)
    ...
    if /<input type=hidden name=act value="cmd"><input type=hidden name="d" value="\//.match?(target.body)
      m << { filepath: target.body.scan(/<input type=hidden name=act value="cmd"><input type=hidden name="d" value="([^\"]+)/) } if target.body =~ /<input type=hidden name=act value="cmd"><input type=hidden name="d" value="([^\"]+)/
    end
