...
##
WhatWeb::Plugin.define "SimpleTech-SimpleShare-NAS" do
  ...
  def passive(target)
    ...
    # Check Server
    if /httpd/.match?(target.headers["server"])

      # SimpleShare WWW-Authenticate Realm
      m << { status: 401, name: "SimpleShare WWW-Authenticate Realm" } if target.headers["www-authenticate"] =~ /Basic realm="SimpleShare \(default user name is admin and password is simple\)"/
