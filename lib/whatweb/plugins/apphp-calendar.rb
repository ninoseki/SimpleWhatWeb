...
##
WhatWeb::Plugin.define "ApPHP-Calendar" do
  ...
  def aggressive(target)
    ...
    if target.body =~ /<!-- This script was generated by ApPHP Calendar v\.([\d\.]+) \(http:\/\/www\.apphp\.com\) -->/ && target.uri.path =~ /\.php/

      # Open application base url + "?view_type[]"
      target_url = target.uri.to_s.scan(/^([^\n]*\.php)/).to_s + "?view_type[]"
      new_target = WhatWeb::Target.new(target_url)

      # Extract local file path # PHP error
      m << { filepath: new_target.body.scan(/: The first argument should be either a string or an integer in (<b>)?([^\n^<]+)(<\/b>)? on line (<b>)?[\d]+(<\/b>)?/)[0][1] } if new_target.body =~ /: The first argument should be either a string or an integer in (<b>)?([^\n^<]+)(<\/b>)? on line (<b>)?[\d]+(<\/b>)?/
