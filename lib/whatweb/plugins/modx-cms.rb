...
WhatWeb::Plugin.define "ModxCMS" do
  ...
  def aggressive(target)
    ...
    v = WhatWeb::Matcher::Version.new("Modx", versions, target.uri)
  ...
  def passive(target)
    m = []
    m << { name: "SN4 Cookie", certainty: 75 } if target.headers["set-cookie"] =~ /^SN4[a-z0-9]{12}=/
    m << { name: "P3P" } if target.headers["P3P"] =~ /CP="NOI NID ADMa OUR IND UNI COM NAV"/
