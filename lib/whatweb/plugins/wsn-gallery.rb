...
##
WhatWeb::Plugin.define "WSN-Gallery" do
  ...
  def passive(target)
    ...
    # Version Detection
    if /<title>WSN Gallery<\/title>/.match?(target.body)

      # Error Page
      if target.body =~ /<p>WSN Gallery is unable to render this page. The reason is described below:/ && target.body =~ /<p>Version info: WSN Gallery ([\d\.]+)<\/p>/
        m << { version: target.body.scan(/<p>Version info: WSN Gallery ([\d\.]+)<\/p>/) }
      end

      # Error Page # no id parameter
      if target.body =~ /<div class="main">Non-numeric id value given in creating object.<\/div>/ && target.body =~ /<p>WSN Gallery ([\d\.]+)<\/p>/
        m << { version: target.body.scan(/<p>WSN Gallery ([\d\.]+)<\/p>/) }
      end
