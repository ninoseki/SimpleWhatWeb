...
##
WhatWeb::Plugin.define "HP-SiteScope" do
  ...
  def passive(target)
    ...
    if /^SiteScope\//.match?(target.headers["server"])
      if target.headers["server"] =~ /^SiteScope\/(.+) [\d]{1,2}:[\d]{2} [ap]m [\d]{1,2}\/[\d]{2}\/[\d]{2} [Bb]uild:? ([\d]+)$/
        m << { version: "#{$1} build #{$2}" }
      elsif target.headers["server"] =~ /^SiteScope\/([^\s]+( [A-Z]\d)?)/
        m << { version: $1.to_s }
      end
    end

    # WWW-Authenticate Header
    if /BASIC realm="SiteScope Administrator"/.match?(target.headers["www-authenticate"])
